...
const proxyUrl = 'https://cors-proxy-11-12-19.herokuapp.com/';
...
for (let i = 0; i < zipCodes.length; i++) {
  const zipUrl = proxyUrl + `https://data.boston.gov/api/3/action/datastore_search_sql?sql=SELECT * from "31358fd1-849a-48e0-8285-e813f6efbdf1" WHERE _full_text @@ to_tsquery('${zipCodes[i]}') LIMIT ${limit}`;
  urls.push(zipUrl);
}
  ...
  fetch(url)
    ...
    .then((data) => {
      ...
      data.forEach((d) => {
        ...
        } else if (d.result.records === undefined) {
          const error = document.getElementById('error-container');
          error.innerHTML = errorMsg;
        }
      });
